...
const fallbackGenerators = [
  {
    ...
    name: "Shapes",
    description:
      "Layered geometric shapes with randomized size, position, and palette color.",
    category: "geometry",
...
function getSelectedGenerator() {
  return (
    state.generators.find(
      (generator) => generator.id === elements.generatorSelect.value,
    ) || state.generators[0]
  );
}
...
function generatorHasParameter(generator, parameterId) {
  return Boolean(
    generator.parameters?.some((parameter) => parameter.id === parameterId),
  );
}
...
function setLoading(isLoading) {
  ...
  elements.generateButtonText.textContent = isLoading
    ? "Generating"
    : "Generate";
}
...
function updateSwatches() {
  const colors =
    paletteColors[elements.colorPalette.value] || paletteColors.mixed;
...
function syncGeneratorControls() {
  ...
  const densityParameter = generator.parameters?.find(
  ...
  if (
    densityParameter?.defaultValue &&
    document.activeElement !== elements.shapes
  ) {
    elements.shapes.value = densityParameter.defaultValue;
    elements.densityRange.value = Math.min(
      densityParameter.defaultValue,
      elements.densityRange.max,
    );
  }
...
function syncCustomResolutionState() {
  ...
  const matchingPreset = Array.from(elements.resolutionPreset.options).find(
    (option) => option.value === current,
  );
  elements.resolutionPreset.value = matchingPreset
...
function syncDensityInputs(source) {
  if (source === "range") {
    ...
    const value = Math.max(
      1,
      Math.min(
        Number(elements.shapes.value || 1),
        Number(elements.shapes.max || 5000),
      ),
    );
    elements.densityRange.value = Math.min(
      value,
      Number(elements.densityRange.max),
    );
  }
}
...
function getSelectedShapeTypes() {
  return Array.from(
    elements.shapeTypeOptions.querySelectorAll(
...
async function handleSubmit(event) {
  ...
  try {
    ...
    const elapsedHeader = Number.parseFloat(
      response.headers.get("x-generation-time-ms"),
    );
    const elapsedMs = Number.isFinite(elapsedHeader)
      ? elapsedHeader
      : performance.now() - startedAt;
    const filename =
      response.headers.get("x-wallpaper-filename") ||
      `${generator.id}_${elements.width.value}x${elements.height.value}_${Date.now()}.png`;
    const seed =
      response.headers.get("x-wallpaper-seed") ||
    ...
    updateDownload(state.previewUrl, filename);
    updateStatus(elapsedMs, { seed });
  } catch (error) {
...
async function loadGenerators() {
  try {
    ...
    state.generators = data.generators?.length
      ? data.generators
      : fallbackGenerators;
  } catch (error) {
