...
async function drawBokeh(ctx, request) {
  ...
  const layers = [
    ...
    {
      count: Math.round(baseCount * 0.38),
      blur: 10,
      minRadius: 0.04,
      maxRadius: 0.1,
      alpha: 0.24,
    ...
    {
      count: Math.round(baseCount * 0.4),
      blur: 4,
      minRadius: 0.015,
      maxRadius: 0.055,
      alpha: 0.32,
  ...
  for (const layer of layers) {
    ...
    for (let i = 0; i < layer.count; i++) {
      ...
      const radius =
        minDimension *
        (layer.minRadius + rng() * (layer.maxRadius - layer.minRadius));
    ...
    const blurredLayerBuffer = await sharp(layerCanvas.toBuffer("image/png"))
      .blur(layer.blur)
      .toBuffer();
