...
const { createDefaultOutputPath } = require('./output');
...
function validateGenerationInput(input = {}) {
    ...
    const seed = input.seed === undefined || input.seed === null || input.seed === '' ? createRandomSeed() : String(input.seed);
    ...
    if (!hasGenerator(generationType)) {
        details.push(`generationType must be one of: ${Object.keys(generators).join(', ')}.`);
    }

    if (!isPaletteName(colorPalette)) {
        details.push(`colorPalette must be one of: ${getPaletteNames().join(', ')}.`);
    }
    ...
    const invalidShapeTypes = normalizedShapeTypes.filter((shapeType) => !SHAPE_TYPES.includes(shapeType));

    if (invalidShapeTypes.length > 0) {
        details.push(`shapeTypes contains unsupported values: ${invalidShapeTypes.join(', ')}.`);
    }
    ...
    const outputFile = input.outputFile || createDefaultOutputPath({ generationType, width, height, seed });
